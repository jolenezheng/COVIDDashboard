...
def get_fit_of_pwpd_vs_scalelength(ax, arr, popdensity):
    ...
    while(True):
        try:
            ...
            # Tricky: Raise my own RuntimeError if fit is bad:
            if (A_err > A):
...
if (Pars.startrow == 0):
    ...
    Pars.pwpd_counties = pd.read_csv(Pars.csv_out_filename)
# loop over counties, mask GHS, calculate pwpd
if (Pars.plot_pwpd):
    plotdir = 'plots'
for index, row in Pars.pwpd_counties.iterrows():
    # allow for arbitrary start position, and
    #   DON'T DO SHELBY COUNTY KENTUCKY!
    # it is fucked up for some reason
