...
def get_fit_of_pwpd_vs_scalelength(arr, countyfips, countyname, popdensity):
    plotdir = 'plots'
    (rows, cols) = arr.shape
        ...
        try:
            ...
            if Pars.fit_with_delta:
                ...
                ax.text(0.1, 0.1, textstring5, horizontalalignment = 'left',
                        verticalalignment='center',transform=ax.transAxes)
            filename = plotdir + '/' + countyfips + '.pdf'
                ...
                if firsttry:
                    ...
                    if (Pars.plot_pwpd):
                        ...
                        ax.text(0.1, 0.1, textstring1, horizontalalignment = 'left',
                                verticalalignment='center',transform=ax.transAxes)
                        filename = plotdir + '/' + countyname + '.pdf'
                        fig.savefig(filename)
    ...
    if (index < 10):
        ...
        if Pars.do_pwpd_vs_scalelength:
            A, A_err, xb, xb_err, al1, al1_err, al2, al2_err, delt, delt_err = \
                get_fit_of_pwpd_vs_scalelength(imarr, statecountyfips,
                                               countyname, popdensity)
